...
import FreelancerInvoicePDF from '@/components/pdf/FreelancerInvoicePDF';
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const data = { 
        ...
        items, currency, notes, subtotal 
      };
      
      const blob = await pdf(<FreelancerInvoicePDF data={data} />).toBlob();
      const url = URL.createObjectURL(blob);
      
      const link = document.createElement('a');
      link.href = url;
      link.download = `Invoice-${invoiceNo || '001'}.pdf`;
